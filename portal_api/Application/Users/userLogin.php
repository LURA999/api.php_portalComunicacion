...
class userLogin extends database{
    ...
    function getLogin($usuario, $contrasena){
        // Parametro de usuario y contrasena
        $usuario = base64_decode($usuario);
        $contrasena = base64_decode($contrasena);

        // Hace query con el metodo siguiente
        $query = $this->getUser($usuario);
        
        // Agarra datos de usuario
        $arr = $query -> fetchAll(PDO::FETCH_ASSOC);
        // Bandera para saber si la contrasena coincide
        $comprobar = false;
            ...
            case 1:
               // $com= " and DATE(fecha) = DATE(now())";
                ...
                if(intVal($tipoVista) == 1) {
                 ...
                 if($hotel == -1){
                    $sql = $this->connect()->prepare("SELECT idUsuario, u.usuario, apellidoPaterno, apellidoMaterno, nombres, correo, cveRol, cveLocal, l.nombre AS local, fechaNacimiento, img, fechaIngreso, dep.departamento, contrato, COUNT(v.fecha) AS fecha, cumple, aniversario
    ...
    function getAllUsersBirth($hotel){
        ...
        FROM usuario INNER JOIN departamentos dep on idDepartamento = cveDepartamento where MONTH(CURRENT_DATE()) = MONTH(fechaNacimiento) and ".(($hotel == 1 || $hotel == 0) ? "(cveLocal = 1 or cveLocal= 0)" : "(cveLocal = :cveLocal)")." AND cumple = 1 order by DAY(fechaNacimiento) asc");
        if( $hotel  >1) {
            $sql->bindParam(":cveLocal",  $hotel, PDO::PARAM_INT);
        }  
        $sql->execute();     
    ...
    function getAllUsersAniv($aniv){
        ...
        FROM usuario INNER JOIN departamentos dep on idDepartamento = cveDepartamento where ".(($aniv == 1 || $aniv == 0) ? "(cveLocal = 1 or cveLocal= 0)" : "cveLocal = :cveLocal")." and  aniversario = 1 and (DATE_ADD(fechaIngreso, INTERVAL 1 YEAR) <= CURRENT_DATE() OR fechaIngreso >= DATE_SUB(CURRENT_DATE(), INTERVAL 1 YEAR)) 
    ...
    function getAllUsersMesi($mesi){
        ...
        if($mesi == 3 || $mesi == 5 ){
            ...
            where fechaInicio <= DATE(now()) and fechaFinal >= DATE(now()) and ".(($mesi == 1 || $mesi == 0) ? "(cveLocal = 1 or cveLocal= 0)" : "cveLocal = :cveLocal")." order by fecha, posicion asc;");
        }else{
            $sql = $this->connect()->prepare("SELECT cveUsuario, nombres,  mesString(fecha) mes, dep.departamento, contrato, c.nombre contratoNombre, img, apellidoPaterno, apellidoMaterno, posicion, cveLocal, fechaInicio, fechaFinal 
            FROM empleado_mes INNER JOIN usuario on cveUsuario = idUsuario INNER JOIN contrato c on contrato = idContrato INNER JOIN departamentos dep on idDepartamento = cveDepartamento
    ...
    function updateUser($input,$modalidad){
        ...
        fechaNacimiento = :fechaNacimiento, fechaIngreso = :fechaIngreso, cveDepartamento = :departamento , contrato = :contrato, cumple = :cumple, aniversario = :aniversario";
        ...
        $sql->bindparam(':aniversario', $input['aniversario'], PDO::PARAM_INT);
        ...
        /*
        $consulta = 'UPDATE `vehicle` 
        SET `type_vh`= :markvh, `model_vh`= :modelvh, `color`= :color, `plates`= :plates  
        WHERE employee_num = :usuario AND local = :local';
        $sql->bindparam(':usuario', $input['usuario'], PDO::PARAM_INT);
        $sql->bindparam(':markvh', $input['markvh'], PDO::PARAM_STR);
        $sql->bindparam(':modelvh', $input['modelvh'], PDO::PARAM_STR);
        $sql->bindparam(':color', $input['color'], PDO::PARAM_STR);
        $sql->bindparam(':plates', $input['plates'], PDO::PARAM_STR);
    ...
    function createUser($input){
        ...
        /*$sql = $this->connect()->prepare('
        INSERT INTO `vehicle`(`employee_num`, `type_vh`, `model_vh`, `color`, `plates`, `local`) 
        VALUES (:usuario, :markvh, :modelvh, :color, :plates, :local)');
        $sql->bindparam(':usuario', $input['usuario'], PDO::PARAM_INT);
        $sql->bindparam(':markvh', $input['markvh'], PDO::PARAM_STR);
        $sql->bindparam(':modelvh', $input['modelvh'], PDO::PARAM_STR);
        $sql->bindparam(':color', $input['color'], PDO::PARAM_STR);
    ...
    function todosUsuarioLocal($palabra,$hotel){
        $like = '%'.$palabra.'%';
        $sql = $this->connect()->prepare('
        SELECT  idUsuario id, concat(nombres, " ",apellidoPaterno," ",apellidoMaterno) nombre
        FROM usuario 
        WHERE '.(($hotel == 1 || $hotel == 0) ? "(cveLocal = 1 or cveLocal= 0)" : "(cveLocal = :cveLocal)").' 
        AND (concat(nombres, " ",apellidoPaterno," ",apellidoMaterno) LIKE :usuario OR usuario LIKE :usuario);');
        
        if( intval($hotel) > 1) {
            $sql->bindParam(":cveLocal",  $hotel, PDO::PARAM_INT);
        } 
        $sql->bindParam(":usuario",  $like, PDO::PARAM_STR);
        $sql->execute();
        return $sql->fetchAll(PDO::FETCH_ASSOC);
    }

    function updateCumple($input){
        $sql = $this->connect()->prepare('UPDATE usuario SET cumple = :cumple WHERE id= :idUsuario');
        $sql->bindparam(':cumple', $input["cumple"], PDO::PARAM_INT);
        $sql->bindparam(':idUsuario', $input["idUsuario"], PDO::PARAM_INT);
        return $sql->execute();
    }

    function updateAniversario($input){
        $sql = $this->connect()->prepare('UPDATE usuario SET aniversario = :aniversario WHERE id= :idUsuario');
        $sql->bindparam(':aniversario', $input["aniversario"], PDO::PARAM_INT);
        $sql->bindparam(':idUsuario', $input["idUsuario"], PDO::PARAM_INT);
        return $sql->execute();
    }
